...
public class PhysicsWorld implements Runnable, ContactListener {
	private World world;
	private HashMap<Body, PhysicsObject> objectConnections; // Speichert, welche
															// PhysicsObjects zu
															// welchen Bodys
															// gehören. Wichtig
															// für die
	...
	public void endContact(Contact contact) {
		PhysicsObject first = objectConnections.get(contact.getFixtureA()
				.getBody());
		PhysicsObject second = objectConnections.get(contact.getFixtureB()
				.getBody());
	...
	public void postSolve(Contact contact, ContactImpulse impulse) {
		...
		PhysicsObject second = objectConnections.get(contact.getFixtureB()
				.getBody());
		first.beginCollision(new CollisionData(first, second,
				impulse.normalImpulses[0]));
		second.beginCollision(new CollisionData(second, first,
				impulse.normalImpulses[0]));
	}
