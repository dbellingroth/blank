...
/**
 * 
 * beim InputHandler meldet man sich als keyPressed-, keyReleased-,
 * mousePressed- oder mouseReleased-Kandidat an. Der betreffende InputListener
 * gibt real_key/button und pseudo_key/button an. Wenn ein Keyboard-Event im
 * InputHandler ankommt, gibt der es weiter, aber mit dem pseudo_key als
 * Argument.
 * 
 * NEEEEEEEEEEIIIIIIIIIIIIIIIIIIN!!!!!! Mir ist gerade eingefallen, dass man den
 * InputHandler einfach jeweils einen pseudo_key geben kann, den man auf andere
 * Werte setzen kann :( NEEEEEEEEEEEIIIIIIIIIIIIIIIIIN!!!!!! ICH BIN
 * DOOOOOOOFFFFFFFFFFFF!!!!!!
 * 
 * 
...
public class InputHandler implements InputListener {
	...
	private HashMap<InputListener, HashMap<Integer, Integer>> keyTable,
			mouseTable;
	...
	public void addKeyPressedListener(InputListener il, int real_key,
			int pseudo_key) {

		if (!keyPressedList.contains(il))
			keyPressedList.add(il);
	...
	public void addKeyReleasedListener(InputListener il, int real_key,
			int pseudo_key) {

		if (!keyReleasedList.contains(il))
			keyReleasedList.add(il);
	...
	public void addMousePressedListener(InputListener il, int real_key,
			int pseudo_key) {

		if (!mousePressedList.contains(il))
			mousePressedList.add(il);
	...
	public void addMouseReleseadListener(InputListener il, int real_key,
			int pseudo_key) {

		if (!mouseReleasedList.contains(il))
			mouseReleasedList.add(il);
